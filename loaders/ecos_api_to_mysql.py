...
# 목표:
# - 한국은행 ECOS API에서 월간 시계열 데이터를 조회
...
SERIES_CONFIGS = [
    ...
    },
    {
        "series_info": {
            ...
            "notes": (
                "ECOS StatisticSearch 적재 | "
                "STAT_CODE=901Y009 | ITEM_CODE1=*0"
            ),
            "source_series_code": "901Y009:*0",
            "source_series_name": "소비자물가지수",
            "transform_code": "ecos_api",
            "transform_name": "RAW",
            "is_transformed": False,
            "source_unit": "index",
...
def build_ecos_url(params, start_row, end_row):
    """
    ...
    return (
        ...
        f"{start_row}/{end_row}/"
        f"{params['STAT_CODE']}/{params['CYCLE']}/"
        f"{params['START_TIME']}/{params['END_TIME']}/"
        f"{params['ITEM_CODE1']}/{params['ITEM_CODE2']}/{params['ITEM_CODE3']}/{params['ITEM_CODE4']}"
    )
...
def fetch_one_page(params, start_row, end_row):
    """
    ECOS 한 페이지 조회
    """
    url = build_ecos_url(params, start_row, end_row)
...
def fetch_all_pages(params):
    ...
    while True:
        ...
        rows = fetch_one_page(params, start_row, end_row)

        row_count = len(rows)
        print(
            f"[조회] STAT_CODE={params['STAT_CODE']} | "
            f"rows {start_row}~{end_row} | 수신 {row_count}건"
        )
...
def load_one_series(source_id, series_info, params):
    rows = fetch_all_pages(params)
    if not rows:
        raise ValueError(
            f"ECOS 응답에 유효 row가 없습니다. "
            f"series_code={series_info['series_code']} | STAT_CODE={params['STAT_CODE']}"
        )
    ...
    if data_df.empty:
        raise ValueError(
            f"ECOS 응답 row는 있었지만 저장할 유효 데이터(date, value)가 없습니다. "
            f"series_code={series_info['series_code']}"
        )

    final_series_info = build_series_info_with_response_meta(series_info, response_meta)

    payload = build_series_payload(
        series_info=final_series_info,
        source_id=source_id,
        data_df=data_df,
    )
...
def main():
    ...
    for config in SERIES_CONFIGS:
        ...
        print(
            f"[시작] series_code={series_info['series_code']} | "
            f"STAT_CODE={params['STAT_CODE']}"
        )
        load_one_series(
            source_id=source_id,
